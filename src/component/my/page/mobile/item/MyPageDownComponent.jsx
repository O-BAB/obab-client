  ...
  return (
    <div className={`${styles?.myPageDownComponent} py-5 px-4`}>
      ...
      <div className={`flex justify-between flex-row px-5 py-3 mx-8`}>
        <div className={`cursor-pointer`} onClick={() => {onClickMyPageMenuBar('interested/board')}}>관심 글</div>
        <div className={`cursor-pointer`} onClick={() => {onClickMyPageMenuBar('write/board')}}>작성 글</div>
        <div className={`cursor-pointer`} onClick={() => {onClickMyPageMenuBar('write/comment')}}>작성 댓글</div>
      </div>
      <Outlet/>
    </div>
  );
